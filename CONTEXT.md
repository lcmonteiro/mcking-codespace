...
- **2026-08-08** — `python/bumpy.py`: classic demoscene bump-mapped sphere (Second Reality "Bumpy" vibes). Per half-pixel ray-sphere with the true sphere normal; bump field is 4 octaves of multiplicative sines `Σ Aᵢ·sin(2πfᵢu+pᵢ)·sin(2πfᵢv+qᵢ)` — periodic in u (seamless texture spin) and its gradient is **analytic**, so the normal perturbation is exact and cheap. Trick: per-pixel per-octave sin/cos terms are precomputed once; spin only rotates the phase (`sin(a+φ)=su·cosφ+cu·sinφ`), so per-frame cost is a handful of mults + 1 sqrt per pixel (renormalise — the perturbed normal length varies with phase). Orbiting light (auto or manual steer) with lambert + Phong s^32 + ambient, rim darkening `I·(0.55+0.45·Nz)`, 38 twinkling background stars, 5 palettes (Ice/Ember/Toxic/Neon/Chrome); `--once` ASCII dump + benchmark (~85 fps headless at 100×60 half-pixels).
...
- `python/bumpy.py` — classic demoscene bump-mapped sphere: per-pixel ray-sphere, analytic-gradient bump perturbation (4 sine octaves, seamless spin), orbiting/manual light, lambert + Phong specular, rim darkening, twinkling stars, 5 palettes, interactive controls
