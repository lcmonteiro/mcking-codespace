...
def get_terminal_size() -> Tuple[int, int]:
    """Get terminal size, fallback to defaults."""
    try:
        import shutil
...
def make_stars(w: int, h: int, n: int, rng: random.Random) -> List[Star]:
    ...
    for _ in range(n):
        x = rng.random() * w
        y = rng.random() * h * 0.72  # keep above horizon
        stars.append(Star(
            x=x, y=y,
...
def make_fireflies(w: int, h: int, n: int, horizon_y: float, rng: random.Random) -> List[Firefly]:
    flies = []
    for _ in range(n):
        x = rng.random() * w
...
def update_scene(scene: Scene, dt: float):
    ...
    if random.random() < dt * 0.3:  # ~0.3 per second
        w, h = scene.W, scene.H
    ...
    for f in scene.fireflies:
        ...
        if f.x < 0 or f.x > scene.W or f.y < 0:
            ...
            f.y = scene.horizon_y * scene.H + random.random() * (1 - scene.horizon_y) * scene.H * 0.4
            f.phase = 0.0
...
def sky_gradient(t: float) -> Tuple[int, int, int]:
    """Sky color based on vertical position (0=top, 1=horizon)."""
    # Deep night blue at top -> deep purple -> orange/red at horizon
    top = (8, 4, 28)
    mid = (20, 12, 50)
    horizon = (60, 35, 80)
...
def render_frame(scene: Scene) -> str:
    ...
    # Initialize screen as list of character-color pairs
    ...
    for star in scene.stars:
        ...
        # Color: mostly white with slight hue variance
    ...
    try:
        while True:
            ...
            if new_w != scene.W or new_h != scene.H:
                # Rebuild scene with new dimensions
            ...
            # Cap dt to avoid spiral after resize
            dt = min(dt, 0.1)
