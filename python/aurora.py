...
@dataclass
class Star:
    """A twinkling star in the arctic sky."""
    x : float
    y : float
    phase : float
    twinkle_speed : float
...
@dataclass
class AuroraCurtain:
    """A single aurora curtain with wave parameters."""
...
def aurora_gradient(t: float, hue_shift: float) -> Tuple[int, int, int]:
    """
    Map *t* ∈ [0,1] to a colour along the aurora spectrum.
    *hue_shift* ∈ [0,1) shifts between green, teal, purple, pink.

    Base aurora colours (all at full saturation ~100, value modulated):
...
def sky_gradient(t: float) -> Tuple[int, int, int]:
    """Sky colour: dark blue at top, near-black at bottom."""
    intensity = 1.0 - t * 0.7
    r = int(10 * intensity)
    g = int(15 * intensity)
    b = int(40 * intensity)
...
def mountain_color(t: float) -> Tuple[int, int, int]:
    """Silhouette with slight blue tint. Darker at base."""
    r = int(15 * (1 - t * 0.3))
    g = int(20 * (1 - t * 0.3))
    b = int(35 * (1 - t * 0.3))
    return (r, g, b)
...
def generate_stars(w: int, h: int, count: int) -> List[Star]:
    """Generate *count* stars distributed across the upper sky."""
    stars: List[Star] = []
    for _ in range(count):
        stars.append(Star(
            x=random.random(),
            y=random.random() * 0.65,
...
def generate_aurora(w: int, h: int, count: int = 3) -> List[AuroraCurtain]:
    """Generate *count* aurora curtains with random parameters."""
    curtains: List[AuroraCurtain] = []
...
def generate_mountains() -> List[Tuple[float, float]]:
    """Generate mountain silhouette points based on layered sine waves."""
...
def generate_snowflakes(count: int) -> List[Snowflake]:
    """Generate *count* falling snowflakes."""
...
def render_frame(
    ...
    """
    Build one frame as a string of ANSI-coloured characters.

    Args:
        w : Screen width in characters.
        h : Screen height in characters.
        stars : List of star objects.
        auroras : List of aurora curtains.
        mountains : Mountain silhouette profile.
        snowflakes : Falling snowflakes.
        shooting_stars : Active shooting stars.
        t : Global time in seconds.

    Returns:
        A single ANSI-escaped string for one frame.
    """
    lines: List[str] = []
    ...
    for curtain in auroras:
        for py in range(h):
            for px in range(w):
                ...
                combined = (wave + wave2 + wave3) * 0.5 + 0.5
                ...
                color_t = combined * 0.6 + 0.2
                color = aurora_gradient(color_t, curtain.hue_shift
                                        + math.sin(t * 0.05 + curtain.offset_x) * 0.1)
    ...
    for py in range(h):
        ...
        for px in range(w):
            ...
            if ny >= mountain_y:
                ...
                if m_idx_int < len(mountains) - 1:
                    m_y = mountains[m_idx_int][1] * (1 - m_frac) \
                           + mountains[m_idx_int + 1][1] * m_frac
                else:
                    m_y = mountains[-1][1]
            ...
            if is_mountain:
                ...
                if aurora_color is not None:
                    line_parts.append(
                        bg(aurora_color[0], aurora_color[1], aurora_color[2]) + " "
                    )
                else:
        ...
        for px in range(w):
            ...
            star_here: Optional[Tuple[int, int, int, str]] = None
            for star in stars:
                spx = int(star.x * w)
                spy = int(star.y * h)
            ...
            shooting_here: Optional[Tuple[int, int, int, str]] = None
            ...
            flake_here: Optional[Tuple[int, int, int, str]] = None
            ...
            elif flake_here is not None:
                char = flake_here[3]
                line_chars.append(
                    bg(bg_color[0], bg_color[1], bg_color[2])
                    + fg(flake_here[0], flake_here[1], flake_here[2]) + char
                )
            else:
                line_chars.append(
                    bg(bg_color[0], bg_color[1], bg_color[2]) + " "
...
def main() -> None:
    """Run the Aurora Borealis animation."""
    try:
        os.system("")
        ...
        while True:
            ...
            new_shooting: List[ShootingStar] = []
            for ss in shooting_stars:
                ss.x += ss.vx
                ss.y += ss.vy
                ss.life -= FRAME_S
            ...
            frame = render_frame(
                w, h, stars, auroras, mountains, snowflakes, shooting_stars, t
            )
